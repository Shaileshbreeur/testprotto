...
// $row_1 = mysqli_fetch_array($result_bike);   
 ...
 $sql_3 = "SELECT credit_avail,end_date  FROM `pro_clean` where user_id = '$cust_id' order by  pro_clean_id desc LIMIT 1";


 $creditleft =  mysqli_query($conn, $sql_3);
...
<?php } ?>
<center><input type="radio" name="bike"  value="other"> Add bike</center> <br/>
</div>

    
<div style="padding-top: 10px;" class='abc'>
<?php  while($rb = mysqli_fetch_assoc($creditleft)) { 

if($rb['credit_avail'] == 0){  ?>
<label> you have used all your credits..</label>

<?php }else{
  ?> 


<button  type="button " onclick="credit_use(this.value)" name="credits" value="<?php echo $rb['credit_avail'].'//one_credit'; ?>" > use from credit : <?php echo $rb['credit_avail'] . "     expire on   ". $rb['end_date']; ?>  </button>
...
<?php 
   ...
   $cid =  $_SESSION['cust_num'];
 $sql1 = "SELECT * FROM `proclean_check` where user_id = ' $cid'";

 $result1 =  mysqli_query($conn, $sql1);

 if(mysqli_num_rows($result1) > 0){}else{?>
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-6 col-md-6 col-sm-6">
                            <div class="dex-box text-primary border-2 counter-box m-b30">
                                ...
                                <h5 class="dlab-tilte  text-uppercase m-a0"> <span class="dlab-tilte-inner skew-title bg-primary p-lr15 p-tb10">Select</span></h5>
                            </div>
                        </div>
                    </div>
                                            <div class="col-lg-3 col-md-3 col-sm-3"></div>
...
echo "<form class='form-horizontal' method='post' action='custom/instamojo/pay.php' enctype='multipart/form-data' onsubmit='disablebtn()'><fieldset>";

echo "<input id='lat' name='lat' type='hidden' /><input id='lng' name='lng' type='hidden' />";
echo "<input id='page_id' name='page_id' type='hidden' value='procleans' />";
echo "<input id='customer' name='customer_id' type='hidden' value='".$cust_id."' />";


echo "<div style='padding-top:10px;' class='form-group row modellist'><div class='col-md-3'></div><div class='col-md-2 control-label' for='make'>Brand:</label></div><div class='col-md-4'>";
...
echo"</br>";
echo "<div class='form-group row modellist_1' id='modellist'><div class='col-md-3'></div><div class='col-md-2 control-label' for='model'>Model:</label></div><div class='col-md-4'>";
...
echo "<div class='p-a30 bg-white m-b30  card_face'>
                    <div class='section-head '>
                       <center> <h2 class='text-uppercase'>Select your Plan to Proceed</h2></center>
                    </div>
     ...
     <input type='radio' id='months_sub' name='months_sub' value='6'>6 credit  
<input type='radio' id='months_sub1' name='months_sub' value='12'>
12 credit 
<input type='radio' id='months_sub1' name='months_sub' value='36'>
36 credit
<input type='radio' id='months_sub1' name='months_sub' value='60'>
60 credit
</br>

                                            <b><strike id='strike_one'> </strike></b>
                                             <span class='pricingtable-bx' id='prodryprice' > </span> 
                                             <span class='pricingtable-type' > Rs</span> </div><br>
                                             <span>Cost Per Wash </span>
                                             <span class='pricingtable-bx' id='perwash'  > </span> 
                                             <span class='pricingtable-type'   > Rs</span>
                                            <div class='pricingtable-title'>
                                                <h2>Bike Wash Subscription</h2>
                                            </div>
...
echo"</br>";
echo "<div class='form-group row' id='pickupaddress'><div class='col-md-3'></div>
...
echo"</br>";
echo "<div class='form-group row'><div class='col-md-3'></div>
<div class='col-md-2'><label for='time'>Time</label> </div>
<div class='col-md-4'>
...
echo"</br>";
echo "<div class='form-group row'><div class='col-md-3'></div>
<div class='col-md-2'><label  for='regno'>Registration No. (optional)</label> </div>
<div class='col-md-4'>
...
echo "<input type='hidden' id='month_subscribtion' name='month_subscribtion' value='' />";
...
echo "<input id='servicetype' name='servicetype' type='hidden' value='procleans' >";
...
echo "</div>";
echo "<div class='Book_Now' style='display: none;'><input id='Book_Now' name='Book_Now' style='float: right; margin-left: 50px;' type='submit' class='btn' value='Book Now' /> </div>";
...
$('.skew-title').on('click', function() {


if(typeof $("input[name='bike']:checked").val() === "undefined")
{
    alert("please select bike and model");
}else{
...
$('input[type=radio][name=months_sub]').change(function() {
...
$('#strike_one').text('1349');
$('#prodryprice').html('1239');
prodryprice = 1239;
$('#month_subscribtion').val(6); 
$('#perwash').html(225); 


}else if(str_1 == '12'){

$('#strike_one').text('2639');
$('#prodryprice').html('2379');
prodryprice = 2379;
$('#month_subscribtion').val(12); 
$('#perwash').html(220); 
...
$('#strike_one').text('7649');
$('#prodryprice').html('6799');
prodryprice =6799; 
$('#month_subscribtion').val(36); 
$('#perwash').html(212); 

}else if(str_1 == '60'){

$('#strike_one').text('11999');
$('#prodryprice').html('10499');
prodryprice =10499; 
$('#month_subscribtion').val(60); 
$('#perwash').html(200); 
